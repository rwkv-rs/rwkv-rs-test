...
def plot_metric(rows: list[dict[str, Any]], metric: str, output: Path, ylabel: str, log_y: bool, scale: float) -> None:
    os.environ.setdefault("MPLCONFIGDIR", "/tmp/matplotlib-task5")
    ...
    for axis, prompt_len in zip(axes, prompt_lens):
        ...
        for label, points in sorted(grouped.items()):
            ...
            axis.plot([p[0] for p in points], [p[1] for p in points], marker="o", linewidth=1.5, label=label)
...
def main() -> None:
    default_root = Path(__file__).resolve().parents[2] / "results" / "task5"
    parser = argparse.ArgumentParser(description="Plot README task 5 llama.cpp benchmark CSV.")
    parser.add_argument("--csv", type=Path, default=default_root / "llama_cpp_task5.csv")
