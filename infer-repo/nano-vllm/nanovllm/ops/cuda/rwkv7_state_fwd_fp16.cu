...
__global__ void __launch_bounds__(_N_, 2) kernel_forward_w0_fp16_dither(
    ...
    const F *__restrict__ const _state_in, F *__restrict__ _state_out, const F *__restrict__ const _r, const F *__restrict__ const _w, const F *__restrict__ const _k, const F *__restrict__ const _v, const F *__restrict__ const _a, const F *__restrict__ const _b,
    ...
    const F* _state_src = _state_in + bbb * C * _N_ + h * _N_ * _N_;
    F* _state_dst = _state_out + bbb * C * _N_ + h * _N_ * _N_;
    constexpr int ldg_size = sizeof(int4) / sizeof(F);
    #pragma unroll
    for (int j0 = 0; j0 < _N_ / ldg_size; j0++){
        int4 state_vec = ((const int4 *)_state_src)[j0 * _N_ + i];
    ...
    for (int j0 = 0; j0 < _N_ / ldg_size; j0++){
        ...
        ((int4 *)_state_dst)[j0 * _N_ + i] = state_vec;
    }
}


void forward_one(int64_t B, int64_t C, int64_t H, at::Tensor &state_in, at::Tensor &state_out, at::Tensor &r, at::Tensor &w, at::Tensor &k, at::Tensor &v, at::Tensor &a, at::Tensor &b, at::Tensor &y, at::Tensor &elapsed_t){
    assert(H * _N_ == C);
    auto stream = at::cuda::getCurrentCUDAStream();
    kernel_forward_w0_fp16_dither<1><<<B * H, _N_, 0, stream>>>(
        B, 1, C, H, 
        (const F*)state_in.data_ptr(),
        (F*)state_out.data_ptr(), 
        (const F*)r.data_ptr(), 
        (const F*)w.data_ptr(),
        (const F*)k.data_ptr(), 
        (const F*)v.data_ptr(),
...
void forward_seq(int64_t B, int64_t T, int64_t C, int64_t H, at::Tensor &state_in, at::Tensor &state_out, at::Tensor &r, at::Tensor &w, at::Tensor &k, at::Tensor &v, at::Tensor &a, at::Tensor &b, at::Tensor &y, at::Tensor &elapsed_t){
    assert(H * _N_ == C);
