...
try:
    from .trace import TRACE, activation, trace, trace_token_ids
except ImportError:
    from trace import TRACE, activation, trace, trace_token_ids
...
_TRACE_PREFILL_CONTRACT = """
trace_token_ids
trace("embedding", target, outputs="embedding/embedded_context.safetensors")
trace(f"cells/cell_{layer:04d}/time_mixer", target, outputs={0: f"cells/cell_{layer:04d}/time_mixer/embedded_context.safetensors"})
trace(f"cells/cell_{layer:04d}/channel_mixer", target, outputs=f"cells/cell_{layer:04d}/channel_mixer/embedded_context.safetensors")
trace("lm_head", target, outputs="lm_head/logits.safetensors")
"""
...
class RWKV_x070(MyModule):
    ...
    def forward(self, idx, state, full_output=False, with_sampling=False): # will modify state in-place
        if type(idx) is list:
            if len(idx) > 1:
                if TRACE:
                    return self.forward_seq_trace(torch.tensor(idx, device=self.z['emb.weight'].device), state, full_output)
    ...
    def forward_seq_trace(self, idx:torch.Tensor, state:List[torch.Tensor], full_output:bool=False):
        with torch.no_grad():
            z = self.z
            trace_token_ids(idx)
            x = trace("embedding", lambda: z['emb.weight'][idx], idx, outputs="embedding/embedded_context.safetensors")
            ...
            for layer in range(self.n_layer):
                ...
                xx, v_first = trace(
                    ...
                    lambda: RWKV_x070_TMix_seq(layer, self.n_head, self.head_size, xx, state[3*layer], v_first, state[3*layer+1],
                    ...
                    xx,
                    v_first,
                    outputs={0: f"cells/cell_{layer:04d}/time_mixer/embedded_context.safetensors"},
                )
                x = x + xx
                ...
                xx = trace(
                    ...
                    lambda: RWKV_x070_CMix_seq(xx, state[3*layer+2], z[ffn+'x_k'], z[ffn+'key.weight'], z[ffn+'value.weight']),
                    xx,
            ...
            return trace("lm_head", lambda: F.linear(x, z['head.weight']), x, outputs="lm_head/logits.safetensors")

    def forward_seq_batch_trace(self, idxs:List[List[int]], state:List[torch.Tensor], full_output:bool=False):
        idx_tensor = torch.tensor(idxs, device=self.z['emb.weight'].device)
        with torch.no_grad():
            z = self.z
            trace_token_ids(idx_tensor)
            x = trace("embedding", lambda: z['emb.weight'][idx_tensor], idx_tensor, outputs="embedding/embedded_context.safetensors")
            ...
            for layer in range(self.n_layer):
                ...
                xx, v_first = trace(
                    f"cells/cell_{layer:04d}/time_mixer",
                    lambda: RWKV_x070_TMix_seq_batch(layer, self.n_head, self.head_size, xx, state[3*layer], v_first, state[3*layer+1],
                        z[att+'x_r'], z[att+'x_w'], z[att+'x_k'], z[att+'x_v'], z[att+'x_a'], z[att+'x_g'],
                        z[att+'w0'], z[att+'w1'], z[att+'w2'], z[att+'a0'], z[att+'a1'], z[att+'a2'], z[att+'v0'], z[att+'v1'], z[att+'v2'],
                        z[att+'g1'], z[att+'g2'], z[att+'k_k'], z[att+'k_a'], z[att+'r_k'],
                        z[att+'receptance.weight'], z[att+'key.weight'], z[att+'value.weight'], z[att+'output.weight'],
                ...
                xx = trace(
                    f"cells/cell_{layer:04d}/channel_mixer",
                    lambda: RWKV_x070_CMix_seq_batch(xx, state[3*layer+2], z[ffn+'x_k'], z[ffn+'key.weight'], z[ffn+'value.weight']),
                    xx,
                    outputs=f"cells/cell_{layer:04d}/channel_mixer/embedded_context.safetensors",
                )
                x = x + xx
