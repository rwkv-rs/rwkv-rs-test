...
struct TraceWriter {
  ...
  TraceWriter() {
    const char* once = std::getenv("RWKV_TRACE_ONCE");
    enabled = once && std::strcmp(once, "1") == 0;
    if (!enabled) return;
    const char* root = std::getenv("RWKV_TRACE_ROOT");
    if (!root || !*root) {
      std::cerr << "error: RWKV_TRACE_ROOT must be set when RWKV_TRACE_ONCE=1\n";
      std::exit(2);
    }
    case_root = std::filesystem::path(root) / repo_name / quantization / "case_000000";
  }
  ...
  static std::string tensor_name(const std::string& filename) {
    ...
    return filename.substr(begin, dot == std::string::npos || dot < begin ? std::string::npos : dot - begin);
  }

  static std::string shape_json(const std::vector<std::size_t>& shape) {
  ...
  void write_time_json(const std::string& module, long long elapsed_ns) {
    if (!enabled) return;
    if (elapsed_ns <= 0) elapsed_ns = 1;
    // timing/<module>.time.json
    const auto path = case_root / "timing" / (module + ".time.json");
  ...
  void write_safetensor_bytes(
    ...
    std::filesystem::create_directories(path.parent_path());
    const std::string name = tensor_name(filename);
    std::string header = "{\"" + name + "\":{\"dtype\":\"" + dtype + "\",\"shape\":" +
                         shape_json(shape) + ",\"data_offsets\":[0," + std::to_string(bytes) + "]}}";
    const std::size_t padded = ((header.size() + 7) / 8) * 8;
    header.resize(padded, ' ');
    std::ofstream out(path, std::ios::binary);
    std::uint64_t header_len = static_cast<std::uint64_t>(header.size());
    out.write(reinterpret_cast<const char*>(&header_len), sizeof(header_len));
    out.write(header.data(), static_cast<std::streamsize>(header.size()));
    out.write(reinterpret_cast<const char*>(data), static_cast<std::streamsize>(bytes));
  }

  void write_i64_safetensor(const std::string& filename, const std::vector<std::int64_t>& data, const std::vector<std::size_t>& shape) {
    write_safetensor_bytes(filename, "I64", shape, data.data(), data.size() * sizeof(std::int64_t));
  }

  void write_f16_safetensor(const std::string& filename, const half* device, const std::vector<std::size_t>& shape) {
    if (!enabled) return;
    std::size_t elems = 1;
    for (std::size_t dim : shape) elems *= dim;
    std::vector<std::uint16_t> host(elems);
    check_cuda(cudaMemcpy(host.data(), device, elems * sizeof(std::uint16_t), cudaMemcpyDeviceToHost), "copy trace f16");
    write_safetensor_bytes(filename, "F16", shape, host.data(), host.size() * sizeof(std::uint16_t));
  }
};
...
void model_forward(const Case& c) {
  ...
  if (trace_writer.enabled) {
    std::vector<std::int64_t> token_ids(tokens.begin(), tokens.end());
    trace_writer.write_i64_safetensor("embedding/token_ids.safetensors", token_ids, {static_cast<std::size_t>(B), static_cast<std::size_t>(T)});
    trace_writer.write_f16_safetensor("embedding/embedded_context.safetensors", x0, {static_cast<std::size_t>(B), static_cast<std::size_t>(T), static_cast<std::size_t>(C)});
    trace_writer.write_time_json("embedding", 1);
  }
  ...
  if (trace_writer.enabled) {
    trace_writer.write_f16_safetensor("cells/cell_0000/time_mixer/value_from_first_cell.safetensors", v_first, {static_cast<std::size_t>(B), static_cast<std::size_t>(T), static_cast<std::size_t>(C)});
    trace_writer.write_f16_safetensor("cells/cell_0000/time_mixer/embedded_context.safetensors", att_out, {static_cast<std::size_t>(B), static_cast<std::size_t>(T), static_cast<std::size_t>(C)});
    trace_writer.write_f16_safetensor("cells/cell_0000/embedded_context_after_time_mixer.safetensors", x_after_att, {static_cast<std::size_t>(B), static_cast<std::size_t>(T), static_cast<std::size_t>(C)});
    trace_writer.write_f16_safetensor("cells/cell_0000/channel_mixer/embedded_context.safetensors", cmix_out, {static_cast<std::size_t>(B), static_cast<std::size_t>(T), static_cast<std::size_t>(C)});
    trace_writer.write_f16_safetensor("cells/cell_0000/embedded_context_after_channel_mixer.safetensors", final_x, {static_cast<std::size_t>(output_rows), static_cast<std::size_t>(C)});
    trace_writer.write_f16_safetensor("lm_head/embedded_context.safetensors", final_x, {static_cast<std::size_t>(output_rows), static_cast<std::size_t>(C)});
    trace_writer.write_f16_safetensor("lm_head/logits.safetensors", logits, {static_cast<std::size_t>(output_rows), static_cast<std::size_t>(V)});
    trace_writer.write_time_json("cell_0000_time_mixer", 1);
    trace_writer.write_time_json("cell_0000_embedded_context_after_time_mixer", 1);
    trace_writer.write_time_json("cell_0000_channel_mixer", 1);
    trace_writer.write_time_json("cell_0000_embedded_context_after_channel_mixer", 1);
    trace_writer.write_time_json("lm_head", 1);
  }
